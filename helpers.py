...
def display_world(world_size, position, landmarks=None):
    ...
    if(landmarks is not None):
        ...
        for i in range(len(landmarks)):#Added by user
            if([landmarks[i][0],landmarks[i][1]] != position):#Added by user
                ax.text(landmarks[i][0], landmarks[i][1], 'x', ha='center', va='center', color='purple', fontsize=20)#Added by user
                ax.text(landmarks[i][0]-1, landmarks[i][1]+1, i, ha='center', va='center', color='purple', fontsize=20)#Added by user
...
def make_data(N, num_landmarks, world_size, measurement_range, motion_noise, 
    ...
    display_world(int(world_size), [r.x, r.y], r.landmarks)#Added by user
